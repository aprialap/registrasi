...
@section('title')
    <title>Admin | Tambah Jadwal Praktek</title>    
@endsection
@section('content')
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Tambah Jadwal Praktek</h6>
    </div>
    <div class="card-body">
            ...
            <div class="col-4"> 
                <div class="mb-3">
                    ...
                    <select name="poli" class="form-control form-control-sm input-table select2" required>
                        ...
                        @foreach ($poli as $pol)
                            <option value="{{$pol->id}}">{{$pol->nama}}</option>
                        @endforeach
                     </select>
                </div>
            </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="dokter" class="form-control form-control-sm input-table select2" required>
                        ...
                        @foreach ($dokter as $dktr)
                            <option value="{{$dktr->id}}">{{$dktr->nama}}</option>
                        @endforeach
                     </select>
                </div>
            </div>
